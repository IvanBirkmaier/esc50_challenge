...
class RandomMute:
    def __init__(self, min_mute=0.0, max_mute=0.3):
        super(RandomMute, self).__init__()
        self.min_mute = min_mute
        self.max_mute = max_mute

    def mute(self, wave):
        mute_val = random.uniform(self.min_mute, self.max_mute)
        start = random.randint(0, wave.shape[0] - int(mute_val * wave.shape[0]))
        end = start + int(mute_val * wave.shape[0])
        wave[start:end] = 0
        return wave
...
class RandomFrequencyMask:
    def __init__(self, max_width=30, numbers=1):
        super(RandomFrequencyMask, self).__init__()
        self.max_width = max_width
        self.numbers = numbers

    def addFreqMask(self, spec):
        for _ in range(self.numbers):
            mask_len = random.randint(0, self.max_width)
